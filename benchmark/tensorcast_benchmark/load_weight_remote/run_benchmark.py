...
class BenchmarkConfig(BaseModel):
    ...
    weight_version: int = Field(default=0, ge=0)
    ...
    brainctl_charged_group: str = "tensorcast_dev"
    ...
    def validate_tensorcast_fields(self) -> "BenchmarkConfig":
...
def build_paths(run_id: str, uv_project_root: str) -> BenchmarkPaths:
    ...
    chosen_uv_root = (
        Path(uv_project_root).expanduser().resolve()
        if uv_project_root
        else discover_uv_project_root(benchmark_root)
    )
...
def launch_worker(
    ...
    *,
    ...
    completed = run_local(cmd, check=True)
...
def global_is_stopped(status_text: str) -> bool:
    lowered = status_text.lower()
    return "global store session: unknown" in lowered or "unknown" in lowered
...
def start_global_store(
    ...
    exec_user(
        config,
        worker.process_name,
        build_service_remote_cmd(paths, "start-global", str(config_path)),
        check=True,
    )
    wait_for_condition(
        ...
        check_fn=lambda: (
            global_is_ready(
                status := exec_user(
                    config,
                    worker.process_name,
                    build_service_remote_cmd(paths, "status-global"),
                    check=False,
                ).stdout
            ),
            status,
...
def stop_global_store(config: BenchmarkConfig, paths: BenchmarkPaths, worker: WorkerInfo) -> None:
    exec_user(
        config,
        worker.process_name,
        build_service_remote_cmd(paths, "stop-global"),
        check=False,
    )
...
def stop_daemon(config: BenchmarkConfig, paths: BenchmarkPaths, worker: WorkerInfo) -> None:
    exec_user(
        config,
        worker.process_name,
        build_service_remote_cmd(paths, "stop-daemon"),
        check=False,
    )
...
def build_tensorcast_configs(
    ...
    return ServiceLogs(
        global_store_data_log=global_store_log,
        daemon_a_data_log=daemon_a_log,
    )


def build_daemon_b_config(
    config: BenchmarkConfig,
    paths: BenchmarkPaths,
    run_id: str,
    worker_a: WorkerInfo,
    ...
    daemon_cfg = load_yaml(Path(config.tensorcast_daemon_config))
    daemon_b_log = Path(f"/data/{run_id}_trial{trial_id:03d}_worker_b_daemon_b.log")
    daemon_cfg["server"]["listen"]["port"] = config.tensorcast_daemon_port
    daemon_cfg["server"]["advertise"]["host"] = worker_b.ip
    daemon_cfg["server"]["p2p_listen"]["port"] = config.tensorcast_daemon_p2p_port
    daemon_cfg["high_availability"]["global_store_endpoints"][0]["host"] = worker_a.ip
    daemon_cfg["high_availability"]["global_store_endpoints"][0]["port"] = config.tensorcast_global_store_port
    daemon_cfg["observability"]["logging"]["file"] = str(daemon_b_log)
    config_path = paths.generated_configs_dir / f"daemon_b_trial{trial_id:03d}.yaml"
    dump_yaml(config_path, daemon_cfg)
    return config_path, daemon_b_log
...
def clear_model_cache(
    ...
    log(f"Clearing remote filesystem cache on {worker.process_name} for {config.model_path}")
    cache_script = paths.scripts_dir / "clear_remote_fs_cache.sh"
    args = ["bash", str(cache_script), "--path", config.model_path]
...
def publish_artifact(
    ...
    publish_script = paths.scripts_dir / "publish_weight.py"
    publish_log = paths.logs_dir / "worker_a_publish.log"
    ...
    remote_cmd = (
        ...
        f"--weight-version {config.weight_version} "
...
def run_remote_trial(
    ...
    *,
    ...
    trial_log = paths.logs_dir / f"trial{trial_id:03d}_worker_b_sglang.log"
    ...
    remote_cmd = (
        ...
        f"--trial-timeout-s {config.trial_timeout_s} "
        f"--log-path {shlex.quote(str(trial_log))} "
        f"--output-json {shlex.quote(str(trial_json))} "
    ...
    if config.load_format == "tensorcast":
        ...
        if config.tensorcast_disk_fallback_auto_put:
            remote_cmd += "--tensorcast-disk-fallback-auto-put "
        if config.tensorcast_trace_tp_slices:
            remote_cmd += "--tensorcast-trace-tp-slices "

    completed = exec_user(
        config,
        worker_b.process_name,
        remote_cmd,
        timeout_s=config.trial_timeout_s + 300.0,
        check=False,
    )
    if not trial_json.exists():
        raise RuntimeError(
            f"trial result json missing for trial {trial_id}; returncode={completed.returncode}\n"
            f"stdout:\n{completed.stdout}\n"
            f"stderr:\n{completed.stderr}"
        )
    ...
    if completed.returncode != 0 and payload.get("status") == "success":
        ...
        payload["error_message"] = (
            f"remote trial command returned non-zero: returncode={completed.returncode}"
        )
    return payload
...
def build_parser(default_global_cfg: Path, default_daemon_cfg: Path) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark remote SGLang load latency on brainctl workers. "
    ...
    parser.add_argument("--brainctl-charged-group", default="tensorcast_dev")
    parser.add_argument("--brainctl-private-machine", default="group")
    parser.add_argument(
        "--brainctl-mount",
        default="juicefs+s3://oss.i.shaipower.com/step2-alignment-jfs:/mnt/step2-alignment-jfs",
    )
    ...
    parser.add_argument(
        "--tensorcast-tp-slice-plan-cache-dir",
        default="/tmp/sglang_tensorcast_trace_cache",
    )
...
def parse_config(paths: BenchmarkPaths) -> BenchmarkConfig:
    parser = build_parser(
        paths.configs_dir / "global_store_config.yaml",
        paths.configs_dir / "store_daemon_config.yaml",
    )
...
def relaunch_worker_a_until_distinct(
    config: BenchmarkConfig,
    run_id: str,
    worker_b: WorkerInfo,
    ...
    while attempts < config.different_node_max_attempts:
        ...
        process_name = launch_worker(
            ...
            negative_tags=",".join(
                item
                for item in (
                    config.worker_a_negative_tags.strip(),
                    f"node/{worker_b.node}",
                )
                if item
            ),
        )
        ...
        log(
            f"worker A landed on same node as worker B ({info.node}); relaunching worker A"
        )
...
def main() -> None:
    ...
    log(f"Run id: {run_id}")
    log(
        f"Benchmark start: load_format={config.load_format}, topology_mode={config.topology_mode}, "
        f"tp_size={config.tp_size}, trials={config.trials}"
    )
    ...
    service_logs = ServiceLogs(global_store_data_log=None, daemon_a_data_log=None)
    artifact_id = ""
    global_store_address = ""
    daemon_a_address = ""
    ...
    try:
        if config.existing_worker_b_process.strip():
            ...
            log(
                f"Reusing worker B: process={worker_b.process_name}, node={worker_b.node}, ip={worker_b.ip}"
            )
        else:
        ...
        if config.load_format == "tensorcast":
            if config.existing_worker_a_process.strip():
                ...
                log(
                    f"Reusing worker A: process={worker_a.process_name}, node={worker_a.node}, ip={worker_a.ip}"
                )
                if config.topology_mode == "relay" and worker_a.node == worker_b.node:
                    raise RuntimeError(
                        "existing worker A and worker B are on the same node; relay mode requires different nodes"
                    )
            else:
            ...
            service_logs = build_tensorcast_configs(config, paths, run_id, worker_a, worker_b)
            ...
            start_global_store(
                ...
                paths,
                worker_a,
                paths.generated_configs_dir / "global_store.yaml",
            )
            ...
            start_daemon(
                ...
                paths,
                worker_a,
                paths.generated_configs_dir / "daemon_a.yaml",
                global_store_address,
            )
            log("Publishing weights on worker A")
            artifact_id = publish_artifact(config, paths, worker_a, daemon_a_address, run_id)
            log(f"Published artifact_id={artifact_id}")

        for trial_id in range(1, config.trials + 1):
            log(f"Trial {trial_id}/{config.trials} start")
            clear_model_cache(config, paths, worker_b)
            daemon_b_address = ""
            daemon_b_log: Path | None = None
            if config.load_format == "tensorcast" and config.topology_mode == "relay":
                assert worker_a is not None
                daemon_b_cfg, daemon_b_log = build_daemon_b_config(
                    config,
                    paths,
                    run_id,
                    worker_a,
            ...
            elif config.load_format == "tensorcast":
                daemon_b_address = daemon_a_address

            payload = run_remote_trial(
                config,
                paths,
                worker_b,
                trial_id=trial_id,
            ...
            if config.load_format == "tensorcast" and config.topology_mode == "relay":
                ...
                if daemon_b_log is not None:
                    copy_if_exists(
                        daemon_b_log,
                        paths.logs_dir / f"trial{trial_id:03d}_worker_b_daemon_b.log",
                    )
            ...
            result = TrialResult(
                ...
                port=config.port,
                load_time_s=payload.get("load_time_s"),
                ready_time_s=payload.get("ready_time_s"),
            ...
            log(
                f"Trial {trial_id} end: status={result.status}, load_time_s={result.load_time_s}, "
                f"ready_time_s={result.ready_time_s}"
            )
            if result.status != "success":
                raise RuntimeError(f"trial {trial_id} failed: {result.error_message}")
        ...
        if (
            worker_b is not None
            and launched_worker_b
            and config.load_format == "tensorcast"
            and config.topology_mode == "relay"
            and not config.keep_workers
        ):
            with contextlib.suppress(Exception):
                stop_daemon(config, paths, worker_b)

...
def daemon_handle_socket_path(paths, port: int) -> str:
    return str(
        paths.workspace_root.parent / ".tensorcast-kv-sockets" / f"{port}.sock"
    )
...
def build_tensorcast_backend_extra_config(
    *,
    ...
    return {
        ...
        "namespace": f"share_local:{run_id}",
        "engine": "sglang",
        "model_id": model_id,
...
def collect_logs(paths, run_id: str, config: BenchmarkConfig) -> None:
...
def main() -> None:
    ...
    try:
        ...
        collect_logs(paths, run_id, config)

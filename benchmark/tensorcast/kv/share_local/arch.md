...
- `rid`, `engine_request_id`, or Tensorcast programmable instance-step APIs.
...
If the backend does not yet support functional prefix share, the benchmark may
still be useful as a harness and orchestration testbed, but TTFT improvement is
not expected.
...
This is the future target backend for SGLang HiCache integration.
...
- Once SGLang Tensorcast HiCache integration lands, the same benchmark should
  measure functional prefix share without changing the benchmark structure.

Until that integration lands, the Tensorcast path is primarily a topology and
...
`benchmark/tensorcast/kv/` must be extended beyond a simple non-streaming
wrapper.
...
`benchmark/tensorcast/load_weight_remote`.
...
- `benchmark/tensorcast/kv/share_local/README.md`
- `benchmark/tensorcast/kv/share_local/arch.md`
- `benchmark/tensorcast/kv/share_local/scripts/`
- `benchmark/tensorcast/kv/share_local/outputs/`

The benchmark-specific directory should only contain logic unique to
`share_local`.

## 12. Shared Utilities Under `benchmark/tensorcast/kv/`
...
- `benchmark/tensorcast/kv/sgl_client.py`
  - async HTTP client
  - readiness checks
  - streaming `/generate`
  - TTFT measurement helpers
- `benchmark/tensorcast/kv/dataset.py`
  - load prompts from JSONL
  - select first `N` questions
- `benchmark/tensorcast/kv/remote.py`
  - remote worker lifecycle helpers
  - `brainctl launch`, `exec`, `cleanup`, log collection
- `benchmark/tensorcast/kv/outputs.py`
  - run-id creation
  - output directory creation
  - CSV append helpers
- `benchmark/tensorcast/kv/models.py`
...
- avoid `rid` and request-transfer semantics
...
- [x] Create the shared `benchmark/tensorcast/kv/` utility layout.
- [x] Add `benchmark/tensorcast/kv/models.py` with Pydantic config and result models.
- [x] Add `benchmark/tensorcast/kv/outputs.py` for run-id generation, output paths, and CSV helpers.
- [x] Add `benchmark/tensorcast/kv/dataset.py` to load `data/test.jsonl` and select the first `N` prompts.
- [x] Extend dataset loading to support schema-aware LongBench inputs plus coarse min/max prompt-length filtering.
- [x] Add `benchmark/tensorcast/kv/remote.py` for `brainctl` worker launch, exec, cleanup, and remote log collection.
- [x] Add `benchmark/tensorcast/kv/sgl_client.py` with readiness checks and streaming `/generate` support.
- [x] Implement TTFT extraction in the shared SGLang client from streamed first-token arrival.
- [x] Create the `benchmark/tensorcast/kv/share_local/` benchmark skeleton with `run_benchmark.py`, `README.md`, `scripts/`, and `outputs/`.
...
- [ ] Implement Tensorcast global store startup and shutdown for the `hicache-storage-backend=tensorcast` path.
- [ ] Implement Tensorcast daemon startup and shutdown for `tensorcast-daemon-mode=share`.
- [ ] Implement Tensorcast daemon startup and shutdown for `tensorcast-daemon-mode=separate`.
...
- [ ] Validate the `tensorcast share` path as a service-lifecycle harness.
